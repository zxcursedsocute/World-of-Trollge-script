...
getgenv().AutoCraftWater = false
getgenv().AutoCraftHeart = false
...
local HeadPath = game.Players.LocalPlayer.Character.Head
...
local Section = Tab:NewSection("Inf Hp(works only on trolls that have second phase)")

Section:NewToggle("Inf hp", "ToggleInfo", function(state)
   getgenv().InfHp = state
   if state then
       infhp();    
   end
...
function infhp()
   while wait() do
       if not InfHp then break end
       firetouchinterest(HeadPath, game:GetService("Workspace")["Mean brick"], 0)
       firetouchinterest(HeadPath, game:GetService("Workspace")["Mean brick"], 1)
       firetouchinterest(HeadPath, game:GetService("Workspace")["Mean brick"], 0)
       firetouchinterest(HeadPath, game:GetService("Workspace")["Mean brick"], 1)
       for i,v in pairs (game:GetService("Players").LocalPlayer.Backpack:GetDescendants()) do
       if v.Name == "workspacedelete" and v.Parent.ClassName == "Tool" then
       v.Parent.Parent = game.Players.LocalPlayer.Character
   end
end
...
Section:NewToggle("Water Cup", "ToggleInfo", function(state)
    getgenv().AutoCraftWater = state
    if state then
        autocraftwater();    
    end
end)

function autocraftwater()
    spawn(function()
        while wait() do
            fireproximityprompt(game:GetService("Workspace")["Red tree dimension"]["Water Obby"].Water["Water cup"].ProximityPrompt)
            if not AutoCraftWater then game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace["Safe area"].CFrame break end
            game.Players.localPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace")["Red tree dimension"]["Water Obby"].Water["Water cup"].CFrame
            for i,v in pairs (game.Players.LocalPlayer.Backpack:GetDescendants()) do
...
Section:NewToggle("Hearty Cup", "ToggleInfo", function(state)
    getgenv().AutoCraftHeart = state
    if state then
        autocraftheart();    
    end
end)

function autocraftheart()
    spawn(function()
        while wait() do
            fireproximityprompt(game:GetService("Workspace")["Red tree dimension"].HeartyArea.HeartyStand.HeartyCupPart.ProximityPrompt)
            if not AutoCraftHeart then game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace["Safe area"].CFrame break end
            game.Players.localPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace")["Red tree dimension"].HeartyArea.HeartyStand.HeartyCupPart.CFrame
            for i,v in pairs (game.Players.LocalPlayer.Backpack:GetDescendants()) do
                if v.Name == "empty cup" then
                    wait(0.1)               
                    v.Parent = game.Players.LocalPlayer.Character
...
Section:NewToggle("Oil", "ToggleInfo", function(state)
